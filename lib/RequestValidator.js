...
function validate(req, res, next) {
    ...
    if(accesstoken && student) {
        logger.info("Validating request. User: " + student.slice(0, 10) + "[...]. Session " + accesstoken.slice(0, 10) + "[...]", {flowid: req.flowid});
        identification.identificationModel.findOne({studentid: student}, {}, { sort: {'createdAt': -1}}, function(err, result) {
            ...
            if(result) {
                if(result.jsession === accesstoken) {
                    logger.info("Found user " + student.slice(0, 10) + "[...]. Authorized", {flowid: req.flowid});
                    next();
                }
                else {
                    logger.info("Validating request. Comparing sessions " + result.jsession.slice(0, 10) + "[...] <==> " + accesstoken.slice(0, 10) + "[...]", {flowid: req.flowid});
        ...
        if(!accesstoken) {
            logger.error('Missing Accesstoken', {flowid: req.flowid});
        }
        if(!student) {
            logger.error('Missing Student ID', {flowid: req.flowid})
        }
        res.status(403).send("Missing accesstoken or student ID", {flowid: req.flowid});
    }
}
